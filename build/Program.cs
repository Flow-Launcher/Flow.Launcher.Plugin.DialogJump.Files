...
public class BuildContext(ICakeContext context) : FrostingContext(context)
{
    ...
    public const string SlnFile = "../Flow.Launcher.Plugin.DialogJump.Files.sln";
...
public sealed class BuildTask : FrostingTask<BuildContext>
{
    ...
    {
        System.Collections.Generic.IEnumerable<SolutionProject> projects = context.DefaultSln.Value.Projects.Where(p => p.Name.EndsWith("DialogJump.Files"));
        string projectPath = projects.First().Path.FullPath;
...
public class PublishTask : FrostingTask<BuildContext>
{
    public override void Run(BuildContext context)
    {
        SolutionProject project = context.DefaultSln.Value.Projects.First(p => p.Name.EndsWith("DialogJump.Files"));
        DirectoryPath srcDir = project.Path.GetDirectory().Combine(new DirectoryPath("bin/Publish"));
        string dstDir =
            $"{srcDir.GetParent().GetParent().GetParent().FullPath}/{context.PublishDir}";
        ...
        FilePathCollection files = context.GetFiles($"{srcDir}/**/*");
        FilePath? versionFile = null;
        foreach (FilePath? f in files)
        {
            string fStr = f.ToString();
            string fName = f.GetFilename().ToString();
            string fFolder = GetLastFolder(fStr);
...
public sealed class CleanTask : FrostingTask<BuildContext>
{
    public override void Run(BuildContext context)
    {
        foreach (SolutionProject? project in context.DefaultSln.Value.Projects)
        {
